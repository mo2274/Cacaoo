...
public class HomeController : Controller
{
    ...
    public HomeController(AppDbContext cc, IConfiguration config, IWebHostEnvironment iwebHostEnvironment)
    {
        context = cc;
        _config = config;
    ...
    public void LoadReport(DateTime FromDate, DateTime ToDate)
    {
        try
        {
            var orders = _orderService.GetReportOrders(FromDate, ToDate, "E");
            var orderDetails = _orderService.GetReportOrderDetails(FromDate, ToDate, "E");
            ...
            var total_downloads = deviceService.GetTotalDownloads(FromDate,ToDate);
            ViewBag.Total_Downloads = total_downloads.Count;
            var total_monthly_downloads = deviceService.GetTotalMonthlyDownloads(FromDate, ToDate);
