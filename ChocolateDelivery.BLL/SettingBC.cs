...
{
    public class SettingBC
    {
        private ChocolateDeliveryEntities context;
        private string logPath;

        public SettingBC(ChocolateDeliveryEntities benayaatEntities, string errorLogPath = "")
        {
        ...
        public APP_SETTINGS SaveSetting(APP_SETTINGS userDM)
        {
            try
            {
                var user = (from o in context.app_settings
                ...
                {
                    context.app_settings.Add(userDM);
                    context.SaveChanges();
