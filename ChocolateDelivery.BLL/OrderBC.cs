...
namespace ChocolateDelivery.BLL
{
	public class OrderBC
	{
		...
		public OrderBC(ChocolateDeliveryEntities chocolateDeliveryEntities)
		{
		...
		public TXN_Orders SaveOrder(TXN_Orders CustomerDM)
		{
			try
			{
				...
				{
					var Customer = (from o in context.txn_orders
				...
				{
					context.txn_orders.Add(CustomerDM);
					context.SaveChanges();
				}
			}
		...
		public TXN_Order_Details SaveOrderDetail(TXN_Order_Details CustomerDM)
		{
			try
			{
				...
				{
					var Customer = (from o in context.txn_order_details
				...
				{
					context.txn_order_details.Add(CustomerDM);
					context.SaveChanges();
				}
			}
		...
		public TXN_Order_Detail_AddOns SaveOrderDetailAddon(TXN_Order_Detail_AddOns CustomerDM)
		{
			try
			{
				...
				{
					var Customer = (from o in context.txn_order_detail_addons
				...
				{
					context.txn_order_detail_addons.Add(CustomerDM);
					context.SaveChanges();
				}
			}
        ...
        public TXN_Order_Detail_Catering_Products SaveOrderDetailCateringProduct(TXN_Order_Detail_Catering_Products CustomerDM)
        {
            try
            {
                ...
                {
                    var Customer = (from o in context.txn_order_detail_catering_products
                ...
                {
                    context.txn_order_detail_catering_products.Add(CustomerDM);
                    context.SaveChanges();
                }
            }
        ...
        public TXN_Orders? GetOrder(long order_id, string lang = "E")
		{
			...
			try
			{
				...
				var query = (from o in context.txn_orders
							 from s in context.sm_order_status
							 from p in context.sm_payment_types
							 join u in context.sm_restaurant_branches on o.Branch_Id equals u.Branch_Id into branch
							 from u in branch.DefaultIfEmpty()
							 join d in context.app_users on o.Driver_Id equals d.App_User_Id into driver
							 from d in driver.DefaultIfEmpty()
							 join r in context.sm_restaurants on o.Restaurant_Id equals r.Restaurant_Id into restaurant
				...
				if (query != null)
				{
					...
					{
						var useraddressDM = appUserBC.GetUserAddress((long)area.Address_Id);
		...
		public TXN_Order_Details? GetOrderDetail(long order_detail_id)
		{
			...
			try
			{
				...
				area = (from o in context.txn_order_details
		...
		public TXN_Orders? GetOrderByOrderDetail(long order_detail_id)
		{
			...
			try
			{
				area = (from od in context.txn_order_details
						from o in context.txn_orders
		...
		public List<TXN_Order_Details> GetOrderDetails(long order_id)
		{
			...
			try
			{
				query = (from o in context.txn_order_details
		...
		public List<TXN_Order_Detail_AddOns> GetOrderDetailAddOns(long order_detail_id)
		{
			...
			try
			{
				var addons = (from o in context.txn_order_detail_addons
							  from p in context.sm_product_addons
        ...
        public List<TXN_Order_Detail_Catering_Products> GetOrderDetailCategoryProducts(long order_detail_id,string lang ="E")
        {
            ...
            try
            {
                var addons = (from o in context.txn_order_detail_catering_products
                              from p in context.sm_product_catering_products
							  from c in context.sm_products
        ...
        public long GetNextOrderNo()
		{
			...
			try
			{
				var max_no = (from o in context.txn_orders
		...
		public TXN_Order_Logs CreateOrderLog(TXN_Order_Logs CustomerDM)
		{
			try
			{
				...
				{
					var Customer = (from o in context.txn_order_logs
				...
				{
					context.txn_order_logs.Add(CustomerDM);
					context.SaveChanges();
				}
			}
		...
		public Site_Configuration? GetSiteConfiguration(string config_name)
		{
			...
			try
			{
				site_config = (from o in context.site_configuration
		...
		public List<SM_Payment_Types> GetPaymentTypes()
		{
			...
			try
			{
				userMenu = (from o in context.sm_payment_types
							where o.Show
							select o).ToList();
		...
		public PAYMENTS CreatePayment(PAYMENTS paymentDM)
		{
			try
			{
				if (paymentDM.Id != 0)
				{
					var Promoter = (from o in context.payments
				...
				{
					context.payments.Add(paymentDM);
					context.SaveChanges();
				}
			}
			catch (Exception ex)
		...
		public PAYMENTS? GetPaymentByTrackId(string track_id)
		{
			...
			try
			{
				...
				post = (from o in context.payments
		...
		public List<TXN_Orders> GetPendingDriverOrders(string lang, long driver_id)
		{
			...
			try
			{
				var orders = (from o in context.txn_orders
							  join u in context.sm_restaurant_branches on o.Branch_Id equals u.Branch_Id into branch
							  from u in branch.DefaultIfEmpty()
							  join l in context.txn_order_logs on o.Order_Id equals l.Order_Id into log
				...
				foreach (var order in orders)
				{
					if (order.l == null)
					{
						...
						{
							var useraddressDM = appUserBC.GetUserAddress((long)orderDM.Address_Id);
							if (useraddressDM != null)
							{
								address = "Area:" + useraddressDM.Area_Name + " ,Block:" + useraddressDM.Block + " ,Street:" + useraddressDM.Street;
								if (!string.IsNullOrEmpty(useraddressDM.House_No))
		...
		public List<TXN_Order_Details> GetReportOrderDetails(DateTime FromDate, DateTime ToDate, string lang)
		{
			...
			try
			{
				query = (from o in context.txn_orders
						 from od in context.txn_order_details
						 from s in context.sm_order_status
						 join u in context.sm_restaurant_branches on o.Branch_Id equals u.Branch_Id into branch
						 from u in branch.DefaultIfEmpty()
						 where (o.Payment_Type_Id == PaymentTypes.Cash || (o.Payment_Type_Id == PaymentTypes.KNET && o.Status_Id != OrderStatus.ORDER_FAILED && o.Status_Id != OrderStatus.ORDER_PROCESSING_PAYMENT))
			...
			try
			{
				...
				if (dashboard_order_type == DashboardOrderTypes.OFD_30)
				{
					orders = (from io in orders
							  from l in context.txn_order_logs
				...
				if (dashboard_order_type == DashboardOrderTypes.DUPLICATE)
				{
					var innerQuery = (from io in orders
									  from l in context.txn_order_details
				...
				foreach (var order in orders)
				{
					...
					orderDM.Order_Id = order.o.Order_Id;
					orderDM.Order_No = order.o.Order_Serial;
					orderDM.Order_Time = order.o.Order_Datetime.ToString("hh:mm tt");
					orderDM.Restaurant_Name = order.r.Restaurant_Name_E;
					if (order.u != null)
		...
		public TXN_Order_Tracking_Details CreateOrderTrackingDetail(TXN_Order_Tracking_Details CustomerDM)
		{
			try
			{
				...
				{
					var Customer = (from o in context.txn_order_tracking_details
				...
				{
					context.txn_order_tracking_details.Add(CustomerDM);
					context.SaveChanges();
				}
			}
		...
		public TXN_Order_Tracking_Details? GetLatestTrackingDetail(long order_id)
		{
			...
			try
			{
				site_config = (from o in context.txn_order_tracking_details
							   where o.Order_Id == order_id
							   orderby o.Track_Datetime descending
							   select o).FirstOrDefault();
			}
